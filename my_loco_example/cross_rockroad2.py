...
class CrossRockroad2(Node):
    def __init__(self):
        super().__init__("cross_rockroad2")
        ...
        self.declare_parameter("step_height", 0.11)
        self.declare_parameter("target_x", 2.55)
        self.declare_parameter("max_abs_y", 0.35)
        self.declare_parameter("max_time", 40.0)
        ...
        self.get_logger().info(
            "cross_rockroad2 | speed=%.2f | step_height=%.2f | target_x=%.2f"
            % (self.forward_speed, self.step_height, self.target_x)
        )
    ...
    def control_loop(self):
        ...
        if x >= self.target_x:
            ...
            self.get_logger().info(
                "state=FINAL_STOP | reason=target_x | pose=(%.3f, %.3f, %.3f)"
                % (x, y, z)
            )
            return
